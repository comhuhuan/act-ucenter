...
public class PunycodeUtil {
	...
	private static String punys_to_ocdn(String punys) throws PunyException {
		...
		String rds = punys;
		String tds = "", rtds_u = "", resu_out = "", strpuny = "";
		while (rds.indexOf(dndot) != -1) {
			tds = rds.substring(0, rds.indexOf(dndot));
			rds = rds.substring(rds.indexOf(dndot) + 1);
			if (!(tds.equalsIgnoreCase(""))) {
				/** update for xn--��Сд�޹� */
				if (isdomain(tds)
						&& ((tds.length() < puny_prefix.length()) || (!tds
								.substring(0, puny_prefix.length())
								.equalsIgnoreCase(puny_prefix)))) {
					rtds_u = tds;
				} else {
					try {
		...
		if (!(rds.equalsIgnoreCase(""))) {
			if (isdomain(tds)
					&& ((rds.length() < puny_prefix.length()) || (!rds
							.substring(0, puny_prefix.length())
							.equalsIgnoreCase(puny_prefix)))) {
				rtds_u = rds;
			}
